...
At the beginning of the evaluation, you will come across 78 texts but do not be discouraged. In total you have to review ALL 78, but there is 6 unique texts along with their 13 respective shortened summaries.\n
...
if user_name:
    ...
    if 'user_data' in st.session_state: # and not st.session_state['all_processed']
        ...
        # Outside the "Next" button click event
        if 'selected_index' in st.session_state and st.session_state['selected_index'] == len(user_data):
            ...
            if submit_button:
                user_data.at[
                    st.session_state[
                        'selected_index'] - 1, 'User Opinion'] = user_opinion if user_opinion else "No opinion given"
                # Save the updated data to the worksheet
                save_data("sorted_FINAL_DATA", st.session_state['user_name'], user_data)
